...
@pytest.mark.asyncio
async def test_referral_success(db_session, services):
    """
    ...
    * Validación:
    ...
    # Create a base rank (0 points) that should exist
    base_rank = Rank(
        name="Bronze",
        min_points=0,
...
@pytest.mark.asyncio
async def test_referral_fraud(db_session, services):
    """
    ...
    * Escenario B: Usuario A intenta invitar a Usuario B que YA existe en el sistema.
    ...
    # Check that no profile was created for the user
    result = await db_session.execute(
        select(GamificationProfile).where(GamificationProfile.user_id == user_id)
    )
    profile = result.scalar_one_or_none()
    
    # Profile should be None since referral failed and no initial profile existed
    # Actually, the process_referral function will call get_or_create_profile which creates it
    # So let's update the test to check that no points were added due to failed referral
...
@pytest.mark.asyncio
async def test_daily_cooldown(db_session, services):
    """
